...
// параметры разбиения выгрузки на тома (Неопределено - многотомность не активна)
Перем мПараметрыМноготомности;

// Процедура создает файл выгрузки
//
// Параметры
//   ИмяФайла - имя файла выгрузки
//   ТолькоПроверкаНедопустимыхСимволов - Булево - режим проверки без создания файла
//   ФильтрСоставаВыгрузки - Строка, Неопределено - если задан, выгружается только
//        указанная часть состава: полное имя объекта метаданных ("Справочник.Номенклатура"),
//        "#Константы" (все константы состава) или "#ДополнительныеОбъекты"
//        (только таблица дополнительных объектов). Используется при выгрузке
//        в отдельные файлы по объектам метаданных.
//
Процедура ВыполнитьВыгрузку(Знач ИмяФайла, ТолькоПроверкаНедопустимыхСимволов = Ложь, Знач ФильтрСоставаВыгрузки = Неопределено) Экспорт

	ОбъектыВыгруженныеСОшибками = Новый Соответствие;

	ОбработанныхКонстант = 0;
	ОбработанныхНаборовЗаписей = 0;
	мПараметрыМноготомности = Неопределено;

	СоставВыгрузки();

	ПрименитьФильтрСоставаВыгрузки(ФильтрСоставаВыгрузки);

	ВыгружатьДополнительныеОбъекты = (ФильтрСоставаВыгрузки = Неопределено
		ИЛИ ФильтрСоставаВыгрузки = "#ДополнительныеОбъекты");

	Если СоставПолнойВыгрузки.Количество() = 0
		И (НЕ ВыгружатьДополнительныеОбъекты
			ИЛИ ДополнительныеОбъектыДляВыгрузки.Количество() = 0) Тогда
	...
	Если ТолькоПроверкаНедопустимыхСимволов Тогда
		...
		ВыгрузкаДанных(ЗаписьXML, ТолькоПроверкаНедопустимыхСимволов, ОбъектыВыгруженныеСОшибками, ВыгружатьДополнительныеОбъекты);

	Иначе

		Если МноготомнаяВыгрузка Тогда

			мПараметрыМноготомности = НовыеПараметрыМноготомности(ИмяФайла);
			ЗаписьXML = НачатьФайлВыгрузки(ИмяОчередногоТома());

		Иначе

			ЗаписьXML = НачатьФайлВыгрузки(ИмяФайла);
		...
		ВыгрузкаДанных(ЗаписьXML, , , ВыгружатьДополнительныеОбъекты);

		ЗавершитьФайлВыгрузки(ЗаписьXML);
...
//
Процедура ВыгрузкаДанных(ЗаписьXML, ТолькоПроверкаНедопустимыхСимволов = Ложь, ОбъектыВыгруженныеСОшибками = Неопределено, Знач ВыгружатьДополнительныеОбъекты = Истина)
	...
	Попытка
		...
		Если ВыгружатьДополнительныеОбъекты Тогда
...
Процедура ЗаписьДанныхОбъектногоТипа(СтрокаДереваМетаданных, ЗаписьXML, ОбъектыВыгруженныеСОшибками, ТолькоПроверкаНедопустимыхСимволов = Ложь)
	...
	Если МноготомнаяВыгрузка И НЕ ТолькоПроверкаНедопустимыхСимволов Тогда

		ЗаписьДанныхОбъектногоТипаПорциями(СтрокаДереваМетаданных, ЗаписьXML, ОбъектыВыгруженныеСОшибками);
		Возврат;
...
// Процедура выгружает данные объектного типа порциями: сначала запросом выбираются
// только ссылки, затем данные читаются и записываются пачками ограниченного размера.
// Применяется при многотомной выгрузке, чтобы не материализовывать в памяти
// результат запроса по всей таблице.
//
// Параметры
//   СтрокаДереваМетаданных - строка дерева метаданных выгружаемого объекта
//   ЗаписьXML - объект, через который происходит запись объектов ИБ
//
Процедура ЗаписьДанныхОбъектногоТипаПорциями(СтрокаДереваМетаданных, ЗаписьXML, ОбъектыВыгруженныеСОшибками)

	ПостроительОтчета = ПодготовитьПостроительДляВыгрузки(СтрокаДереваМетаданных, "_.Ссылка");
	ПостроительОтчета.Выполнить();
	ВыборкаСсылок = ПостроительОтчета.Результат.Выбрать();

	ИмяДляЗапроса = СтрокаДереваМетаданных.ЭлементОписания.ДляЗапроса + СтрокаДереваМетаданных.ОбъектМД.Имя;

	РазмерПорции = ЛимитОбъектовВТоме();

	МассивСсылокПорции = Новый Массив;
	Пока ВыборкаСсылок.Следующий() Цикл

		МассивСсылокПорции.Добавить(ВыборкаСсылок.Ссылка);

		Если МассивСсылокПорции.Количество() >= РазмерПорции Тогда

			ВыгрузитьДанныеМассиваСсылок(МассивСсылокПорции, ИмяДляЗапроса, ЗаписьXML, Ложь, ОбъектыВыгруженныеСОшибками);
			МассивСсылокПорции = Новый Массив;
...
Процедура ВыполнитьВспомогательныеДействияДляЗаписиXML(ВсегоОбработаноОбъектов, ЗаписьXML, ТолькоПроверкаНедопустимыхСимволов)
	...
	Если Не ТолькоПроверкаНедопустимыхСимволов Тогда
		...
		// процедура вызывается перед записью каждого верхнеуровневого объекта,
		// поэтому смена тома происходит строго на границе объектов
		Если мПараметрыМноготомности <> Неопределено Тогда

			Если мПараметрыМноготомности.ЗаписаноВТекущийТом >= ЛимитОбъектовВТоме() Тогда

				ЗавершитьФайлВыгрузки(ЗаписьXML);
				ИнициализироватьТаблицуПредопределенных();

				ЗаписьXML = НачатьФайлВыгрузки(ИмяОчередногоТома());
				мПараметрыМноготомности.ЗаписаноВТекущийТом = 0;

			КонецЕсли;

			мПараметрыМноготомности.ЗаписаноВТекущийТом = мПараметрыМноготомности.ЗаписаноВТекущийТом + 1;
...
Процедура ЗаписьЧерезНаборЗаписей(ЗаписьXML, МенеджерНабораЗаписей, ДляЗапроса, ИмяОбъекта, 
	...
	Если КоличествоПолейОтбора = 0
		И МноготомнаяВыгрузка
		И НЕ ТолькоПроверкаНедопустимыхСимволов Тогда

		ШаблонСообщения = Нстр("ru = 'Регистр %1%2 не имеет полей отбора и будет выгружен целиком одним набором записей (без разбиения на порции)'");
		СообщитьПользователю(ПодставитьПараметрыВСтроку(ШаблонСообщения, ДляЗапроса, ИмяОбъекта));
...
////////////////////////////////////////////////////////////////////////////////
// ВЫГРУЗКА В НЕСКОЛЬКО ФАЙЛОВ
//
// Поддерживаются два независимых (и совместимых друг с другом) режима:
//
// 1. ВыгружатьВОтдельныеФайлыПоМетаданным - каждый объект метаданных выгружается
//    в отдельный файл: 00001.Справочник_Контрагенты.xml, 00001.Документ_ПКО.xml...
//    Константы группируются в файл 00001.Константы.xml, дополнительные объекты -
//    в файл 00001.ДополнительныеОбъекты.xml. Оркестровка (вызов на каждый файл)
//    выполняется формой или процедурой ВыполнитьМногофайловуюВыгрузку().
//
// 2. МноготомнаяВыгрузка - поток выгрузки разбивается на тома: каждые
//    КоличествоОбъектовВТоме объектов текущий файл корректно закрывается
//    (со своим блоком предопределенных) и открывается следующий:
//    00001.001.xml, 00001.002.xml... Данные при этом читаются порциями,
//    а не всей таблицей разом, что ограничивает расход памяти.
//
// Каждый создаваемый файл - полноценная самостоятельная выгрузка, которая
// загружается обычной процедурой ВыполнитьЗагрузку().

// Открывает файл выгрузки и записывает заголовок формата
//
// Параметры
//   ИмяФайла - Строка - полное имя создаваемого файла
//
// Возвращаемое значение:
//   ЗаписьXML, ЗаписьFastInfoset - объект записи открытого файла
//
Функция НачатьФайлВыгрузки(Знач ИмяФайла)
...
// Записывает завершение формата (блок предопределенных) и закрывает файл выгрузки
//
// Параметры
//   ЗаписьXML - объект записи открытого файла
//
Процедура ЗавершитьФайлВыгрузки(ЗаписьXML)

	ЗаписьXML.ЗаписатьКонецЭлемента(); //V8Exch:Data
	ВыгрузитьТаблицуПредопределенных(ЗаписьXML);
	ЗаписьXML.ЗаписатьКонецЭлемента(); //V8Exch:_1CV8DtUD
	ЗаписьXML.Закрыть();

КонецПроцедуры

// Возвращает структуру параметров многотомной выгрузки по базовому имени файла
//
Функция НовыеПараметрыМноготомности(Знач ИмяФайла)
	...
	Иначе
		...
		БазовоеИмя = Лев(ИмяФайла, СтрДлина(ИмяФайла) - СтрДлина(Расширение));

	КонецЕсли;

	Параметры = Новый Структура;
	Параметры.Вставить("БазовоеИмя", БазовоеИмя);
	Параметры.Вставить("Расширение", Расширение);
	Параметры.Вставить("НомерТома", 0);
	Параметры.Вставить("ЗаписаноВТекущийТом", 0);

	Возврат Параметры;

КонецФункции

// Увеличивает номер тома и возвращает имя файла очередного тома
//
Функция ИмяОчередногоТома()

	мПараметрыМноготомности.НомерТома = мПараметрыМноготомности.НомерТома + 1;

	НомерСтрокой = Формат(мПараметрыМноготомности.НомерТома, "ЧЦ=3; ЧВН=; ЧГ=");

	Возврат мПараметрыМноготомности.БазовоеИмя + "." + НомерСтрокой + мПараметрыМноготомности.Расширение;

КонецФункции

// Возвращает действующий лимит объектов в одном томе (он же размер порции чтения)
//
Функция ЛимитОбъектовВТоме()

	Если КоличествоОбъектовВТоме > 0 Тогда
		Возврат КоличествоОбъектовВТоме;
	КонецЕсли;
...
// Оставляет в составе полной выгрузки только строки, соответствующие фильтру.
// Состав вспомогательной выгрузки (по ссылке) не изменяется, поэтому выгрузка
// ссылок-зависимостей продолжает работать в каждом отдельном файле.
//
// Параметры
//   ФильтрСоставаВыгрузки - Строка, Неопределено - см. ВыполнитьВыгрузку()
//
Процедура ПрименитьФильтрСоставаВыгрузки(Знач ФильтрСоставаВыгрузки)

	Если ФильтрСоставаВыгрузки = Неопределено Тогда
		Возврат;
	КонецЕсли;

	УдаляемыеСтроки = Новый Массив;

	Для Каждого СтрокаСостава Из СоставПолнойВыгрузки Цикл

		Если ФильтрСоставаВыгрузки = "#Константы" Тогда
			СтрокаПодходит = Метаданные.Константы.Содержит(СтрокаСостава.ОбъектМД);
		ИначеЕсли ФильтрСоставаВыгрузки = "#ДополнительныеОбъекты" Тогда
			СтрокаПодходит = Ложь;
		Иначе
			СтрокаПодходит = (СтрокаСостава.ОбъектМД.ПолноеИмя() = ФильтрСоставаВыгрузки);
		КонецЕсли;

		Если НЕ СтрокаПодходит Тогда
			УдаляемыеСтроки.Добавить(СтрокаСостава);
		КонецЕсли;

	КонецЦикла;

	Для Каждого УдаляемаяСтрока Из УдаляемыеСтроки Цикл
		СоставПолнойВыгрузки.Удалить(УдаляемаяСтрока);
	КонецЦикла;

КонецПроцедуры

// Возвращает массив заданий для выгрузки в отдельные файлы по объектам метаданных.
// Каждое задание - структура: ФильтрСоставаВыгрузки, СуффиксИмениФайла, Представление.
// Если дерево метаданных еще не построено, выполняется Инициализация().
//
// Возвращаемое значение:
//   Массив из Структура - задания в порядке выгрузки
//
Функция ЗаданияМногофайловойВыгрузки() Экспорт
	...
	Для Каждого СтрокаСостава Из СоставПолнойВыгрузки Цикл
		...
		Если Метаданные.Константы.Содержит(СтрокаСостава.ОбъектМД) Тогда
			ЕстьКонстанты = Истина;
			Продолжить;
		КонецЕсли;

		ПолноеИмя = СтрокаСостава.ОбъектМД.ПолноеИмя();

		Задание = Новый Структура;
		Задание.Вставить("ФильтрСоставаВыгрузки", ПолноеИмя);
		Задание.Вставить("СуффиксИмениФайла", СтрЗаменить(ПолноеИмя, ".", "_"));
		Задание.Вставить("Представление", ПолноеИмя);
	...
	Если ЕстьКонстанты Тогда
		...
		Задание = Новый Структура;
		Задание.Вставить("ФильтрСоставаВыгрузки", "#Константы");
		Задание.Вставить("СуффиксИмениФайла", "Константы");
		Задание.Вставить("Представление", НСтр("ru = 'Константы'"));
		Задания.Вставить(0, Задание);

	КонецЕсли;

	Если ДополнительныеОбъектыДляВыгрузки.Количество() > 0 Тогда

		Задание = Новый Структура;
		Задание.Вставить("ФильтрСоставаВыгрузки", "#ДополнительныеОбъекты");
		Задание.Вставить("СуффиксИмениФайла", "ДополнительныеОбъекты");
		Задание.Вставить("Представление", НСтр("ru = 'Дополнительные объекты'"));
		Задания.Добавить(Задание);
...
// Возвращает имя файла задания многофайловой выгрузки:
// "C:\Выгрузка\00001.xml" + "Справочник_Контрагенты" -> "C:\Выгрузка\00001.Справочник_Контрагенты.xml"
//
// Параметры
//   БазовоеИмяФайла   - Строка - имя файла выгрузки, заданное пользователем
//   СуффиксИмениФайла - Строка - суффикс задания
//
Функция ИмяФайлаЗаданияВыгрузки(Знач БазовоеИмяФайла, Знач СуффиксИмениФайла) Экспорт

	Файл = Новый Файл(БазовоеИмяФайла);
	Расширение = Файл.Расширение;

	Если ПустаяСтрока(Расширение) Тогда

		Основа = БазовоеИмяФайла;
		Расширение = ?(ИспользоватьФорматFastInfoSet, ".fi", ".xml");

	Иначе

		Основа = Лев(БазовоеИмяФайла, СтрДлина(БазовоеИмяФайла) - СтрДлина(Расширение));

	КонецЕсли;

	Возврат Основа + "." + СуффиксИмениФайла + Расширение;

КонецФункции

// Выполняет выгрузку в отдельные файлы по объектам метаданных одним вызовом.
// Предназначена для программного запуска (например, из фонового задания):
//
//   Обработка = Обработки.ВыгрузкаЗагрузкаДанныхXML.Создать();
//   Обработка.ЗагрузитьНастройкиВыгрузкиИзФайла("C:\Настройки.xml");
//   Обработка.ВыполнитьМногофайловуюВыгрузку("C:\Выгрузка\00001.xml");
//
// Примечание: при интерактивной работе форма выгружает каждое задание отдельным
// вызовом сервера, что дополнительно сбрасывает счетчик расхода памяти кластера.
//
// Параметры
//   БазовоеИмяФайла - Строка - базовое имя файлов выгрузки
//
Процедура ВыполнитьМногофайловуюВыгрузку(Знач БазовоеИмяФайла) Экспорт

	Задания = ЗаданияМногофайловойВыгрузки();
	...
	Для Каждого Задание Из Задания Цикл
		...
		ИмяФайлаЗадания = ИмяФайлаЗаданияВыгрузки(БазовоеИмяФайла, Задание.СуффиксИмениФайла);
		ВыполнитьВыгрузку(ИмяФайлаЗадания, Ложь, Задание.ФильтрСоставаВыгрузки);
...
Функция ПолучитьНастройкиВыгрузки() Экспорт
	...
	Настройки.Вставить("ВыгружатьВОтдельныеФайлыПоМетаданным", ВыгружатьВОтдельныеФайлыПоМетаданным);
	Настройки.Вставить("МноготомнаяВыгрузка", МноготомнаяВыгрузка);
	Настройки.Вставить("КоличествоОбъектовВТоме", КоличествоОбъектовВТоме);
...
Процедура УстановитьНастройкиВыгрузки(Знач Настройки) Экспорт
	...
	ИменаПростыхНастроек = "ДатаНачала, ДатаОкончания, ВыгружатьСДокументомЕгоДвижения, ИспользоватьФорматFastInfoSet, ТипВыгрузкиПериодическихРегистров, ИспользоватьОтборПоДатеДляВсехОбъектов, ПриЗагрузкеИспользоватьРежимОбменаДанными, ПродолжитьЗагрузкуВСлучаеВозникновенияОшибки, ВключитьВозможностьРедактированияИспользованияИтогов, ВыгружатьВОтдельныеФайлыПоМетаданным, МноготомнаяВыгрузка, КоличествоОбъектовВТоме";
