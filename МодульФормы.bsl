...
Процедура ВыгрузитьДанные(Команда)
	...
	Если РежимРаботыНаКлиенте
		И (Объект.ВыгружатьВОтдельныеФайлыПоМетаданным ИЛИ Объект.МноготомнаяВыгрузка) Тогда

		ТекстСообщения = Нстр("ru = 'Выгрузка в отдельные файлы по объектам метаданных и многотомная выгрузка
			|поддерживаются только в режиме работы на сервере'");
	...
	Если Объект.ВыгружатьВОтдельныеФайлыПоМетаданным Тогда

		ВыгрузитьДанныеПоМетаданным();
		Возврат;
...
// Выгрузка в отдельные файлы по объектам метаданных: каждое задание (объект
// метаданных, константы, дополнительные объекты) выгружается отдельным вызовом
// сервера, что сбрасывает счетчик расхода памяти кластера между файлами
&НаКлиенте
Процедура ВыгрузитьДанныеПоМетаданным()

	Задания = ПодготовитьЗаданияВыгрузкиНаСервере();
	...
	Для Каждого Задание Из Задания Цикл
		...
		ТекстСостояния = Нстр("ru = 'Выгрузка: %1 (%2 из %3)...'");
		ТекстСостояния = СтрЗаменить(ТекстСостояния, "%1", Задание.Представление);
		ТекстСостояния = СтрЗаменить(ТекстСостояния, "%2", НомерЗадания);
		ТекстСостояния = СтрЗаменить(ТекстСостояния, "%3", Задания.Количество());
		Состояние(ТекстСостояния);

		ВыгрузитьЗаданиеНаСервере(Задание.ФильтрСоставаВыгрузки, Задание.СуффиксИмениФайла);

	КонецЦикла;

	СообщитьПользователю(Нстр("ru = 'Многофайловая выгрузка завершена'"));
...
&НаСервере
Функция ПодготовитьЗаданияВыгрузкиНаСервере()

	ОбъектНаСервере = РеквизитФормыВЗначение("Объект");
	ЗаполнитьДеревоМетаданныхНаСервере(ОбъектНаСервере);

	Возврат ОбъектНаСервере.ЗаданияМногофайловойВыгрузки();

КонецФункции

&НаСервере
Процедура ВыгрузитьЗаданиеНаСервере(ФильтрСоставаВыгрузки, СуффиксИмениФайла)

	ОбъектНаСервере = РеквизитФормыВЗначение("Объект");
	ЗаполнитьДеревоМетаданныхНаСервере(ОбъектНаСервере);

	ИмяФайлаЗадания = ОбъектНаСервере.ИмяФайлаЗаданияВыгрузки(ИмяФайлаВыгрузки, СуффиксИмениФайла);

	ОбъектНаСервере.ВыполнитьВыгрузку(ИмяФайлаЗадания, Ложь, ФильтрСоставаВыгрузки);
